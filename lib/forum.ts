...
export async function listPosts({
  ...
  const raw = snap.docs.map((d) => decodePost(d));

  // Cursor follows the underlying Firestore page, not the filtered set —
  // otherwise we'd skip past the hidden chunks on the next request.
  const lastDoc = snap.docs[snap.docs.length - 1];
  return {
    posts: filterPosts(raw),
    nextCursor: raw.length === pageSize && lastDoc ? lastDoc : null,
  };
}
...
export async function getPost(id: string): Promise<Post | null> {
  ...
  const post = decodePost(snap);
  // Hidden posts surface as 404 to public — admins can still fetch via direct
  // Firestore Console / admin tooling.
  if (isPostHidden(post)) return null;
  return post;
}
...
export async function listComments(postId: string): Promise<Comment[]> {
  ...
  const snap = await getDocs(q);
  return filterComments(snap.docs.map((d) => decodeComment(d, postId)));
}
